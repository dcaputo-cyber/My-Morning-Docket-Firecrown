...
Designed for use with GitHub Pages hosting: the feed URL is public but
effectively unguessable due to a long random token in the repo/path.
...
def main() -> int:
    ...
    ap.add_argument("--base-url", required=True,
                    help="Public URL prefix where the MP3s are hosted.")
    ap.add_argument("--title", default="Morning Docket")
    ap.add_argument("--author", default="Darren Caputo")
    ap.add_argument("--description",
                    default="A daily 25-30 minute audio briefing for the "
                            "general counsel of a media company. "
                            "Privacy, IP, ad tech, publishing, corporate "
                            "governance, legal tech, business AI, and the "
                            "industry angles that matter.")
    ...
    mp3s = sorted(
        ...
        key=lambda p: p.name,
        reverse=True,   # newest first in feed
    )
    ...
    for p in mp3s:
        ...
        guid = f"morning-docket-{date_str}"
